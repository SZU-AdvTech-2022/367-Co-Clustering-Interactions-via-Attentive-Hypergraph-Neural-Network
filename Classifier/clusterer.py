...
class ClusterBaseline(object):
    def __init__(self, embedding_model, input_data, embedding_size):
        ...
        with tf.variable_scope("embedding_model"):
            self.model = Model(input_data, embedding_size)
            self.behavior_embedding, self.entity_embeddings = self.model.logits(entity=True)  # 超边与节点的embedding
            self.N2E_attns = self.model.attn_weights()
        ...
        self.integrated_gradient = tf.zeros_like(self.model.ori_attn)  # 梯度指导

        if FLAGS.coef_grad > 0:
            self.models_grad, self.behavior_embeddings_grad, self.N2E_attns_grad = [], [], []
            with tf.variable_scope("embedding_model", reuse=True):
                for i in range(num_steps):
                    grad_model = Model(input_data, embedding_size, grad_alpha=i/num_steps, silent_print=True)
                    self.models_grad.append(grad_model)
                    self.behavior_embeddings_grad.append(grad_model.logits())
                    weights = grad_model.attn_weights()
                    self.N2E_attns_grad.append(weights)
        ...
        if FLAGS.coef_dualcluster > 0 and FLAGS.coef_grad > 0:
            ...
            for i in range(num_steps):
                with tf.variable_scope("dual_cluster", reuse=True):
                    cluster_output_grad = emb_weight_dual_cluster_module(self.model.num_nodes[0], C_num,
                                                                    ...
                                                                    self.global_step, self.dropout, self.training, self.v)
                    preds = tf.reduce_max(cluster_output_grad[2], axis=-1)  # 预测 每个超边所属类别它的概率
                    first_attn_inputs = self.models_grad[i].first_attn_inputs  # 拼接矩阵 注意力网络的输入
            ...
            with tf.variable_scope("attn_grad"):
                self.attn_grad_loss, integrated_gradient_compressed = gradient_guided_attn_module(
                    ...
                    self.model.sub1_attn_weight_list[target_layer])  # 得到的注意力权重
    ...
    def training_op(self, loss):
        with tf.name_scope('train_optimizer'):
            ...
            grads_and_vars = self.optimizer.compute_gradients(loss, var_list)  # 计算梯度
            capped_grads_and_vars = [(tf.clip_by_value(g, -5., 5.), v) for g, v in grads_and_vars if g is not None]  # 梯度裁剪 控制最大范式
            update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            with tf.control_dependencies(update_ops):
                apply_gradient_op = self.optimizer.apply_gradients(capped_grads_and_vars, global_step=self.global_step)  # 更新
