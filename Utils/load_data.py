...
def load_data(dataset, onehot_for_nofeature=True):
    ...
    # 构建 E4N 和 N4E   4 means "from"   (E4N: 行代表E，列代表N)  行号和数字代表的序号，均从 1 开始，0为预留的pad 同样缩小了邻接阵的特征
    E4N, N4E = [], []
    for e2n in E4N_adjs:
        # 给id补padding
        # 第一行也是padding
        ...
        ans_e4n = pd.DataFrame(ans_e4n).fillna(0).values.astype(np.int64)  # 同样缩小了邻接阵的特征
        ans_n4e = pd.DataFrame(ans_n4e).fillna(0).values.astype(np.int64)
        E4N.append(ans_e4n)
        N4E.append(ans_n4e)

    # 给feature补padding
    # feature矩阵中的值，是从1开始编号的，即feature_index预留了padding
